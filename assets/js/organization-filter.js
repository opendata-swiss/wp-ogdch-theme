 ...
 *
 * Diacritics Map based on
 * https://gist.github.com/yeah/1283961
 * with the adaption that every character is converted
 * to just one character:
 * Before:
 * a with Umlaut, a with Accent, etc. -> ae
 * After:
 * a with Umlaut, a with Accent, etc. -> a
 **/
$(function() {
    ...
    function buildIndex(parentDiv) {
        var out = [];
        parentDiv.find('> ul > .organization').each(function (i, row) {
            row = $(row);
            var link = row.find('> .organization-row > a:first');
            var searchText = removeDiacritics(link.text());
            out.push({
                div: row,
                link: link,
                rawtext: link.text(),
    ...
    function onChange(e) {
        ...
        window.clearTimeout(window.organization_filter_timeout);
        window.organization_filter_timeout = window.setTimeout(function () {
