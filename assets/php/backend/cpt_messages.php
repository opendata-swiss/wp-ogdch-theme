...
function ogd_cpt_updated_messages( $messages ) {
	...
	if ( $post_type_object->publicly_queryable ) {
		...
		$preview_link = sprintf( _x( ' <a target="_blank" href="%1$s">Preview %2$s</a>', '%1$s contains url to preview this post. %2$s contains singular name of the post type.', 'ogdch' ), esc_url( $preview_permalink ), esc_html( $singular ) );
